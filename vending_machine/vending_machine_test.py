...
class VendingMachineTests(unittest.TestCase):
    ...
    def test_should_add_correct_amount_for_one_coin(self):
        for valid_coin in TestsFixture.VALID_COINS:
            ...
            self.vending_machine.insert_coin(valid_coin)
            self.assertEqual(self.vending_machine.get_amount(), value)

    def test_should_not_add_anything_when_incorrect_coin_inserted(self):
        for invalid_coin in TestsFixture.INVALID_COINS:
            self.given_vending_machine()
            self.vending_machine.insert_coin(invalid_coin)
            self.assertEqual(self.vending_machine.get_amount(), 0)
    ...
    def test_should_display_amount_when_more_than_one_valid_coins_inserted(self):
        self.given_vending_machine_with_coins()
        self.assertEqual(self.vending_machine.display_message(), self.get_display_msg_of_expected_amount_of_money())

    def get_display_msg_of_expected_amount_of_money(self):
        return TestsFixture.AMOUNT_TPL % self.expected_amount_of_money
    ...
    def test_should_have_products(self):
        self.assertEqual(self.vending_machine.get_products(), TestsFixture.PRODUCTS)
